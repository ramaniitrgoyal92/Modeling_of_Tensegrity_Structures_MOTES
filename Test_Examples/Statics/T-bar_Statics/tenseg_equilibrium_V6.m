function [Force_den,Support_Load,LABEL,MIN_MASS,Mass_bar,Mass_string,BarWidth_in,StringWidth_in,Loop] = tenseg_equilibrium_V6(tenseg_struct)
...
mass_gamma_new = rho_s/sigma_s*String_len;
mass_lambda_new = 2*rho_b/sqrt(pi*E_b)*Bar_len.^(5/4);
mass_member_new=[mass_gamma_new;mass_lambda_new];
...
Force_den{1}= fmincon(@(x) nonlinear_force_density_V5(x,ns,LABEL,...
    mass_member_new,mass_member),x0,[],[],K1,W_vec,zeros(1,size(C_s,1)))
...
x0=1*10^-2.*ones(ns+nb,1);
options = optimoptions('fmincon','Algorithm','sqp')
Force_den{Loop}= fmincon(@(x) nonlinear_force_density_V5(x,ns,LABEL,...
    mass_member_new,mass_member),x0,[],[],K1,W_vec,zeros(1,size(C_s,1)))
...
MM(1:ns)=mass_member_new(1:ns).*Force_den{Loop}(1:ns); %%%%%%%%%%%%%%%%%%%%%%%%%%

for i=ns+1:length(mass_member_new)
    if LABEL(i-ns)==1;
        MM(i)=mass_member_new(i)*sqrt(Force_den{Loop}(i))
    else
...
[MIN_MASS,FF] = nonlinear_force_density_V5(Force_den{Loop},ns,LABEL,mass_member_new,mass_member);
% save new.mat
%%
...
if max(r_string) - min(r_string) < 10^-6
    StringWidth_in=2*ones(size(r_string));
else
